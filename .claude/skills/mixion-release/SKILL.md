...
2. Backend tests with `--filter "Requires!=AudioDevices"` (runners have no audio devices), then `npm ci` and the frontend unit tests — before the build, so a failing test doesn't waste one.
3. `build.ps1 -Mode portable -Version X.Y.Z` → `output/Mixion.exe`, self-contained, with the version in its file properties and `/api/health`.
...
4. Commit (`Release vX.Y.Z`) and push. The branch is `main`; the GitHub remote is `origin` in this clone (other clones may name it differently — check `git remote -v`):
   ```powershell
...
- **The run failed before publishing** (build, tests): fix it and push the fix. As nothing was published, move the tag to the fixed commit and push it again — `git tag -f vX.Y.Z`, then `git push -f origin vX.Y.Z` — which starts a new run.
